"""
Option B Dashboard - Cincin Api Analysis v2
============================================

Dashboard dengan:
- Label Stadium 1-4 (bukan G1-G4)
- Drilldown per blok
- Deskripsi MAE dan Correlation
"""
...
def run_option_b(df: pd.DataFrame, divisi: str) -> pd.DataFrame:
    ...
    df_result['Stadium'] = 'SEHAT'  # Stadium 1 = Sehat
    ...
    for blok in df_result['Blok'].unique():
        ...
        if mask_mature.sum() > 1:
            ...
            if std_m > 0:
                ...
                # Stadium 3 = Sick (G3)
                df_result.loc[mask_mature & (df_result['Z_Score'] < z_g3_mature), 'Stadium'] = 'SICK'
                # Stadium 2 = Warning (G2)  
                df_result.loc[mask_mature & (df_result['Z_Score'] >= z_g3_mature) & (df_result['Z_Score'] < z_g2_mature), 'Stadium'] = 'WARNING'
        ...
        if mask_young.sum() > 1:
            ...
            if std_y > 0:
                ...
                df_result.loc[mask_young & (df_result['Z_Score'] < z_g3_young), 'Stadium'] = 'SICK'
                df_result.loc[mask_young & (df_result['Z_Score'] >= z_g3_young) & (df_result['Z_Score'] < z_g2_young), 'Stadium'] = 'WARNING'
    ...
    for blok in df_result['Blok'].unique():
        ...
        for (baris, pokok), info in grid.items():
            if info['status'] == 'SICK':
                ...
                sick = sum(1 for db, dp in neighbors if (baris+db, pokok+dp) in grid and grid[(baris+db, pokok+dp)]['status'] in ['WARNING','SICK'])
...
def generate_block_chart(df: pd.DataFrame, blok: str) -> str:
    ...
    colors = {'SEHAT': '#27ae60', 'WARNING': '#f39c12', 'SICK': '#e74c3c'}
    markers = {'MATURE': 'o', 'YOUNG': '^'}
    sizes = {'SEHAT': 30, 'WARNING': 50, 'SICK': 80}
    
    for cat in ['MATURE', 'YOUNG']:
        for status in ['SEHAT', 'WARNING', 'SICK']:
            mask = (df_blok['Category'] == cat) & (df_blok['Stadium'] == status)
            data = df_blok[mask]
            if len(data) > 0:
                label = f'{cat} {status}' if cat == 'MATURE' else f'SISIP {status}'
                ax.scatter(data['N_POKOK'], data['N_BARIS'], 
                          c=colors[status], marker=markers[cat], s=sizes[status],
                          alpha=0.7, label=label)
    
    # Highlight CINCIN_API clusters
    cincin = df_blok[df_blok['Cincin_Status'] == 'CINCIN_API']
    if len(cincin) > 0:
        ax.scatter(cincin['N_POKOK'], cincin['N_BARIS'], 
                  facecolors='none', edgecolors='white', s=150, linewidths=2,
                  label=f'🔥 CINCIN API ({len(cincin)})')
    
    ax.set_xlabel('N_POKOK', fontsize=12)
    ax.set_ylabel('N_BARIS', fontsize=12)
    ax.invert_yaxis()
    
    sehat = len(df_blok[df_blok['Stadium']=='SEHAT'])
    warning = len(df_blok[df_blok['Stadium']=='WARNING'])
    sick = len(df_blok[df_blok['Stadium']=='SICK'])
    cincin_count = len(cincin)
    
    ax.set_title(f'{blok}\nTotal: {len(df_blok):,} | Sehat: {sehat:,} | Warning: {warning:,} | Sick: {sick:,} | 🔥Cincin: {cincin_count}', 
...
def main():
    print("="*70)
    print("OPTION B DASHBOARD v2 - Stadium Labels + Drilldown")
    print("="*70)
    
    output_dir = script_dir / "data" / "output" / "option_b_dashboard_v2"
    ...
    for divisi_code, divisi_name, df_drone in [('AME002', 'AME II', df_ame2), ('AME004', 'AME IV', df_ame4)]:
        ...
        sehat = len(df_result[df_result['Stadium']=='SEHAT'])
        warning = len(df_result[df_result['Stadium']=='WARNING'])
        sick = len(df_result[df_result['Stadium']=='SICK'])
        cincin = len(df_result[df_result['Cincin_Status']=='CINCIN_API'])
        isolated = len(df_result[df_result['Cincin_Status']=='ISOLATED'])
        
        # YOUNG breakdown
        young_df = df_result[df_result['Category']=='YOUNG']
        young_sick = len(young_df[young_df['Stadium']=='SICK'])
        young_warning = len(young_df[young_df['Stadium']=='WARNING'])
        
        results[divisi_code] = {
            'total': total, 'mature': mature_count, 'young': young_count,
            'sehat': sehat, 'warning': warning, 'sick': sick, 
            'cincin': cincin, 'isolated': isolated,
            'young_sick': young_sick, 'young_warning': young_warning
        }
        ...
        # Block drilldown
        block_drilldown[divisi_code] = []
        for blok in df_result['Blok'].unique():
            df_blok = df_result[df_result['Blok'] == blok]
            young_blok = df_blok[df_blok['Category'] == 'YOUNG']
            
            block_data = {
                'blok': blok,
                'blok_norm': normalize_block(blok),
                'total': len(df_blok),
                'young_sisip': len(young_blok),
                'sehat': len(df_blok[df_blok['Stadium'] == 'SEHAT']),
                'warning': len(df_blok[df_blok['Stadium'] == 'WARNING']),
                'sick': len(df_blok[df_blok['Stadium'] == 'SICK']),
                'cincin_api': len(df_blok[df_blok['Cincin_Status'] == 'CINCIN_API']),
                'isolated': len(df_blok[df_blok['Cincin_Status'] == 'ISOLATED']),
            }
            block_data['detected_pct'] = (block_data['warning'] + block_data['sick']) / block_data['total'] * 100 if block_data['total'] > 0 else 0
            block_drilldown[divisi_code].append(block_data)
        
        df_drill = pd.DataFrame(block_drilldown[divisi_code])
        df_merged = df_drill.merge(df_gt_div[['blok_norm', 'gt_pct', 'TOTAL_GANO', 'TOTAL_PKK']], on='blok_norm', how='inner')
        ...
        if len(df_merged) > 0:
            ...
            mae, corr, avg_algo, avg_gt = 0, 0, 0, 0
        ...
        results[divisi_code]['df_merged'] = df_merged
        
        # Generate top block charts
        print(f"  Generating block visualizations...")
        sick_per_block = df_result[df_result['Stadium']=='SICK'].groupby('Blok').size().sort_values(ascending=False)
        top_blocks = sick_per_block.head(6).index.tolist()
        ...
        df_merged.to_csv(output_dir / f"{divisi_code}_drilldown.csv", index=False)
...
<head>
    ...
    <title>Option B Dashboard v2 - Cincin Api</title>
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .stat-label {{ color: #a0a0a0; font-size: 0.85em; margin-top: 5px; }}
        ...
        .block-card {{ background: rgba(255,255,255,0.05); padding: 15px; border-radius: 12px; }}
        .block-card img {{ width: 100%; border-radius: 8px; }}
        .legend {{ background: rgba(255,255,255,0.05); padding: 15px; border-radius: 10px; margin-bottom: 20px; }}
        .legend span {{ display: inline-block; margin-right: 20px; padding: 5px 10px; border-radius: 5px; }}
        table {{ width: 100%; border-collapse: collapse; margin-top: 15px; }}
        th, td {{ padding: 10px; text-align: center; border-bottom: 1px solid rgba(255,255,255,0.1); }}
        th {{ background: rgba(102,126,234,0.2); font-size: 0.9em; }}
        tr:hover {{ background: rgba(255,255,255,0.05); }}
        .metric-desc {{ font-size: 0.85em; color: #a0a0a0; margin-top: 10px; line-height: 1.6; }}
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🅱️ Option B Dashboard v2 - Cincin Api</h1>
            <p>Include YOUNG (Sisip) dengan Threshold Terpisah | Stadium Labels</p>
        ...
        <div class="config-box">
            ...
            <div class="config-grid">
                <div class="config-item">
                    ...
                    Include: MATURE + YOUNG (Sisip)<br>
                    Exclude: DEAD, EMPTY
                </div>
                <div class="config-item">
                    <strong>Label Stadium:</strong><br>
                    SEHAT = Pohon sehat (Z-Score normal)<br>
                    WARNING = Perlu monitoring (Z-Score rendah)<br>
                    SICK = Terindikasi sakit (Z-Score sangat rendah)
                </div>
                <div class="config-item">
                ...
                <div class="config-item">
                    <strong>Cincin Api Status:</strong><br>
                    🔥 CINCIN API = SICK + ≥2 tetangga sakit<br>
                    ISOLATED = SICK tanpa tetangga sakit
                </div>
            </div>
        </div>
"""
    ...
    for divisi_code in ['AME002', 'AME004']:
        ...
        df_drill = r['df_merged']
        
        # MAE interpretation
        if r['mae'] < 3:
            mae_desc = "Sangat Baik - Deteksi algoritma sangat mendekati ground truth"
        elif r['mae'] < 6:
            mae_desc = "Baik - Deteksi cukup akurat"
        elif r['mae'] < 10:
            mae_desc = "Cukup - Ada selisih yang perlu diperhatikan"
        else:
            mae_desc = "Perlu Perbaikan - Selisih besar antara deteksi dan ground truth"
        
        # Correlation interpretation
        if r['corr'] > 0.5:
            corr_desc = "Korelasi Kuat Positif - Pola deteksi sesuai dengan ground truth"
        elif r['corr'] > 0.2:
            corr_desc = "Korelasi Lemah Positif - Ada kesesuaian terbatas"
        elif r['corr'] > -0.2:
            corr_desc = "Tidak Berkorelasi - Deteksi tidak mengikuti pola ground truth"
        else:
            corr_desc = "Korelasi Negatif - Deteksi berkebalikan dengan ground truth"
        ...
        <div class="divisi-section">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-value" style="color: #27ae60;">{r['sehat']:,}</div>
                    <div class="stat-label">SEHAT</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" style="color: #f39c12;">{r['warning']:,}</div>
                    <div class="stat-label">WARNING</div>
                </div>
                <div class="stat-card highlight">
                    <div class="stat-value" style="color: #e74c3c;">{r['sick']:,}</div>
                    <div class="stat-label">SICK</div>
                </div>
                <div class="stat-card highlight">
                    <div class="stat-value" style="color: #e74c3c;">{r['cincin']:,}</div>
                    <div class="stat-label">🔥 CINCIN API</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value" style="color: #95a5a6;">{r['isolated']:,}</div>
            ...
            <div class="config-box" style="border-color: #3498db;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 15px; margin-top: 15px;">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-label">Ground Truth Avg</div>
                    </div>
                </div>
                <div class="metric-desc">
                    <strong>MAE ({r['mae']:.2f}%):</strong> {mae_desc}<br>
                    <strong>Correlation ({r['corr']:.3f}):</strong> {corr_desc}
                </div>
            </div>
            ...
            <div class="config-box" style="border-color: #f39c12;">
                ...
                <p>Total YOUNG: <strong>{r['young']:,}</strong> | 
                   SICK: <strong>{r['young_sick']}</strong> ({r['young_sick']/r['young']*100 if r['young'] > 0 else 0:.1f}%) | 
                   WARNING: <strong>{r['young_warning']}</strong> ({r['young_warning']/r['young']*100 if r['young'] > 0 else 0:.1f}%)
                </p>
            </div>
            
            <h3 style="margin: 20px 0 15px;">📋 Drilldown Per Blok</h3>
...
"""
        ...
        # Add drilldown rows
        for _, row in df_drill.sort_values('sick', ascending=False).head(20).iterrows():
            html += f"""
                    <tr>
                        <td>{row['blok']}</td>
                        <td>{row['total']:,}</td>
                        <td>{row['young_sisip']}</td>
...
"""
        ...
        html += """
            ...
            <h3 style="margin: 25px 0 15px;">🗺️ Top Blocks - Cincin Api Clustering</h3>
            <div class="legend">
                <span style="background: #27ae60; color: white;">● SEHAT</span>
                <span style="background: #f39c12; color: white;">● WARNING</span>
                <span style="background: #e74c3c; color: white;">● SICK</span>
                <span style="border: 2px solid white;">○ Cincin Api</span>
                <span>● MATURE | ▲ SISIP</span>
            </div>
...
"""
    ...
    html_path = output_dir / "option_b_dashboard_v2.html"
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html)
    
    print(f"\n[5/5] Dashboard saved: {html_path}")
